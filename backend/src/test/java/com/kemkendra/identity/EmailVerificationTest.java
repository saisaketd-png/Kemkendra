...
public class EmailVerificationTest {
    ...
    void testBuyerRegistration_requiresEmailVerificationBeforeLogin() throws Exception {
        RegisterRequest registerReq = new RegisterRequest(
                "Unverified Buyer",
                "unverified.buyer@kemkendra.com",
                "+1234567890",
                "Password123!",
                true,
                true
        ...
        mockMvc.perform(post("/api/v1/auth/register")
                ...
                .andExpect(jsonPath("$.email").value("unverified.buyer@kemkendra.com"));

        // Verify email was dispatched
        verify(emailService, times(1)).sendHtmlEmail(
                eq("unverified.buyer@kemkendra.com"),
                eq("[KemKendra] Verify Your Email Address"),
                anyString()
        );

        // Verify token saved in database
        User user = userRepository.findByEmail("unverified.buyer@kemkendra.com").orElseThrow();
        ...
        LoginRequest loginReq = new LoginRequest("unverified.buyer@kemkendra.com", "Password123!");
    ...
    void testVerifyEmail_successAllowsLogin() throws Exception {
        ...
        user.setEmail("candidate@kemkendra.com");
        ...
        mockMvc.perform(post("/api/v1/auth/verify-email")
                ...
                .andExpect(jsonPath("$.message").value("Email verified successfully. Please sign in to your account."))
        ...
        LoginRequest loginReq = new LoginRequest("candidate@kemkendra.com", "Password123!");
        mockMvc.perform(post("/api/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(loginReq)))
                .andExpect(status().isOk())
    ...
    void testVerifyEmail_expiredToken() throws Exception {
        ...
        user.setEmail("expired@kemkendra.com");
        user.setPasswordHash(passwordEncoder.encode("Password123!"));
        user.setRole(UserRole.USER);
        user.setStatus(UserStatus.ACTIVE);
        user = userRepository.save(user);
        ...
        User unverifiedUser = userRepository.findByEmail("expired@kemkendra.com").orElseThrow();
    ...
    void testVerifyEmail_usedToken() throws Exception {
        ...
        user.setEmail("reused@kemkendra.com");
        user.setPasswordHash(passwordEncoder.encode("Password123!"));
        user.setRole(UserRole.USER);
        user.setStatus(UserStatus.ACTIVE);
        user = userRepository.save(user);
    ...
    void testResendVerification_success() throws Exception {
        ...
        user.setEmail("resend@kemkendra.com");
        user.setPasswordHash(passwordEncoder.encode("Password123!"));
        user.setRole(UserRole.USER);
        user.setStatus(UserStatus.ACTIVE);
        user = userRepository.save(user);
        ...
        ResendVerificationRequest resendReq = new ResendVerificationRequest("resend@kemkendra.com");
    ...
    void testResendVerification_throttledWithin60Seconds() throws Exception {
        ...
        user.setEmail("throttled@kemkendra.com");
        user.setPasswordHash(passwordEncoder.encode("Password123!"));
        user.setRole(UserRole.USER);
        user.setStatus(UserStatus.ACTIVE);
        user = userRepository.save(user);
        ...
        ResendVerificationRequest resendReq = new ResendVerificationRequest("throttled@kemkendra.com");
        mockMvc.perform(post("/api/v1/auth/resend-verification")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(resendReq)))
                .andExpect(status().isOk())
    ...
    void testLegacyAndAdminLogin_succeeds() throws Exception {
        ...
        legacyUser.setEmail("legacy@kemkendra.com");
        ...
        LoginRequest legacyLogin = new LoginRequest("legacy@kemkendra.com", "Password123!");
        mockMvc.perform(post("/api/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        LoginRequest adminLogin = new LoginRequest("admin@kemkendra.com", "AdminPass123!");
        mockMvc.perform(post("/api/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
