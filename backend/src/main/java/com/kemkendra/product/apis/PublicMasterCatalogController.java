...
import com.kemkendra.product.LegacyProductTransitionService;
import com.kemkendra.product.MasterProduct;
import com.kemkendra.product.MasterProductService;
import com.kemkendra.product.SupplierOfferingService;
...
public class PublicMasterCatalogController {
    ...
    public ResponseEntity<Page<MasterProductResponse>> searchActiveMasterProducts(
            ...
            @RequestParam(required = false) com.kemkendra.product.ProductCategory category,
        ...
        com.kemkendra.product.dto.MasterProductSearchCriteria criteria = new com.kemkendra.product.dto.MasterProductSearchCriteria(
    ...
    public ResponseEntity<Page<MasterProductResponse>> searchActiveMasterProducts(
            String query,
            com.kemkendra.product.ProductCategory category,
    ...
    public ResponseEntity<List<SupplierOfferingResponse>> getPublicOfferingsForMasterProduct(
        ...
        MasterProduct mp;
        try {
            mp = transitionService.resolveCanonicalMasterProduct(idOrCode);
        } catch (Exception e) {
            return ResponseEntity.ok(List.of());
        }
        if (mp == null || !"ACTIVE".equalsIgnoreCase(mp.getStatus())) {
            return ResponseEntity.ok(List.of());
        }

        List<SupplierOfferingResponse> offerings;
        try {
            offerings = supplierOfferingService.getOfferingsForMasterProduct(mp.getId());
        } catch (Exception e) {
            return ResponseEntity.ok(List.of());
        }
                ...
                .filter(o -> {
                    try {
                        ...
                        return supplierRepository.findById(o.supplierId())
                                .map(s -> {
                                    try {
                                        boolean isVerified = Boolean.TRUE.equals(s.getVerified());
                                        boolean notSuspended = s.getVerificationStatus() != com.kemkendra.seller.SupplierVerificationStatus.SUSPENDED;
                                        boolean notRejected = s.getVerificationStatus() != com.kemkendra.seller.SupplierVerificationStatus.REJECTED;
                                        boolean userOk = s.getUser() == null ||
                                                (s.getUser().getDeletedAt() == null &&
                                                 s.getUser().getStatus() != com.kemkendra.identity.UserStatus.SUSPENDED);
                                        return isVerified && notSuspended && notRejected && userOk;
                                    } catch (Exception ex) {
                                        return false;
                                    }
                                })
