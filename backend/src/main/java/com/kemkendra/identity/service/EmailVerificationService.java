...
public class EmailVerificationService {
    ...
    public VerifyEmailResponse verifyEmail(VerifyEmailRequest request) {
        ...
        // Security reinforcement: Do NOT issue a JWT authentication token upon email verification link click.
        // Possession of the email verification link confirms email reachability, but MUST NOT bypass authentication
        // or allow anyone with access to the outbound mail server/logs to hijack the account without the user's password.
        sendVerificationSuccessNotification(user);

        String role = user.getRole() != null ? user.getRole().name() : "USER";

        log.info("Email successfully verified for user ID: {}, role: {}, supplierStatus: {}", user.getId(), role, verificationStatus);
        return new VerifyEmailResponse("Email verified successfully. Please sign in to your account.", null, role, verificationStatus);
    }

    private void sendVerificationSuccessNotification(User user) {
        try {
            ...
            String subject = "[KemKendra] Security Alert: Your Email Has Been Verified";
            String htmlBody = """
                    <!DOCTYPE html>
                    <html lang="en">
                    <head><meta charset="utf-8"><title>Email Verification Confirmation</title></head>
                    <body style="margin: 0; padding: 0; background-color: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #0f172a;">
                        <table width="100%%" border="0" cellspacing="0" cellpadding="0" style="padding: 48px 16px;">
                            <tr>
                                <td align="center">
                                    <table width="100%%" border="0" cellspacing="0" cellpadding="0" style="max-width: 540px; background-color: #ffffff; border-radius: 8px; border: 1px solid #e2e8f0; padding: 36px 40px;">
                                        <tr>
                                            <td>
                                                <h1 style="margin: 0 0 16px 0; font-size: 18px; font-weight: 600; color: #0f172a;">
                                                ...
                                                <p style="margin: 0 0 20px 0; font-size: 14px; line-height: 22px; color: #334155;">
                                                    Your email address has been successfully verified for your KemKendra account. You can now sign in using your email and password at <a href="%s" style="color: #2563eb; text-decoration: underline;">%s</a>.
                                                </p>
                                                <div style="background-color: #fef2f2; border: 1px solid #fecaca; border-radius: 6px; padding: 12px 16px; margin: 20px 0;">
                                                    <p style="margin: 0; font-size: 13px; line-height: 18px; color: #991b1b; font-weight: 500;">
                                                        Security Notice: If you did not perform this verification, please contact support@kemkendra.online immediately to secure your account.
                                                    </p>
                                                </div>
                    ...
                    """.formatted(recipientName, loginUrl, loginUrl);

            emailService.sendHtmlEmailAsync(user.getEmail(), subject, htmlBody);
        } catch (Exception e) {
            log.warn("Failed to dispatch verification success notification to {}: {}", user.getEmail(), e.getMessage());
        }
    }
