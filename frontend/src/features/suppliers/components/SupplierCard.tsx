...
export function SupplierCard({ supplier }: { supplier: SupplierPublicProfile }) {
  const isVerified = Boolean(supplier.verified);

  return (
    <article className="bg-white border border-[#E2E8F0] rounded-2xl p-5 sm:p-6 hover:border-[#CBD5E1] hover:shadow-tactile-card transition-all flex flex-col justify-between space-y-4 h-full">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-5">
            ...
            <div className="space-y-1.5 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <Link
                  ...
                  className="text-base font-bold text-[#0F172A] hover:text-[#155EEF] transition-colors truncate"
                  title={supplier.name}
                >
                  {supplier.name}
                </Link>
        ...
        <div className="shrink-0 flex flex-col items-stretch sm:items-end gap-3 border-t sm:border-t-0 sm:border-l border-[#E2E8F0] pt-4 sm:pt-0 sm:pl-6 w-full sm:w-auto">
          <Link
            href={`/suppliers/${supplier.id}`}
            ...
            aria-label={`View supplier profile for ${supplier.name}`}
          >
