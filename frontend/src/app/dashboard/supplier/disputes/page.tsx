...
import { getSupplierDisputes } from "@/features/dispute/api/dispute";
...
export default function SupplierDisputesPage() {
  ...
  async function loadDisputes() {
    try {
      setLoading(true);
      setError(null);
      const res = await getSupplierDisputes(statusFilter === "ALL" ? undefined : statusFilter);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">
            Commercial Disputes & Claims
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Review and respond to buyer claims, payment verification notices, and trade issues.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-white">No Inquiries or Disputes</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
            There are currently no active disputes against your chemical orders or invoices.
          </p>
        </div>
      ) : (
                ...
                {disputes.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                    <td className="py-3.5 px-4 font-mono font-bold text-slate-900 dark:text-white">
                      <Link href={`/dashboard/supplier/disputes/${d.id}`} className="hover:text-primary transition">
                        {d.disputeNumber}
                      </Link>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <Link
                        href={`/dashboard/supplier/disputes/${d.id}`}
