...
import { getSupplierInvoices } from "@/features/invoice/api/invoice";
...
export default function SupplierInvoicesPage() {
  ...
  async function loadInvoices() {
    try {
      setLoading(true);
      setError(null);
      const res = await getSupplierInvoices(statusFilter === "ALL" ? undefined : statusFilter);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Track issued tax invoices, reconcile non-custodial buyer payment proofs, and maintain GST records.
          </p>
        </div>
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-500">
          ...
          <p className="text-sm">Loading issued invoices...</p>
        </div>
      ) : invoices.length === 0 ? (
        <div className="p-12 text-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-white">No Issued Invoices Found</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
            You can issue tax invoices directly from your confirmed purchase orders under Orders.
          </p>
          <div className="mt-4">
            <Link
                ...
                {invoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      <p className="text-[11px] font-mono text-slate-500">GSTIN: {inv.buyerGstin || "N/A"}</p>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <Link
                        href={`/dashboard/supplier/invoices/${inv.id}`}
