...
import { getBuyerDisputes } from "@/features/dispute/api/dispute";
...
export default function BuyerDisputesPage() {
  ...
  async function loadDisputes() {
    try {
      setLoading(true);
      setError(null);
      const res = await getBuyerDisputes(statusFilter === "ALL" ? undefined : statusFilter);
      setDisputes(res.content || []);
    } catch (err: any) {
      setError(err.message || "Failed to load commercial disputes");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Track, communicate, and resolve payment or specification disputes with suppliers.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold transition cursor-pointer shadow-sm"
          >
            + Raise Dispute
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-white">No Active Disputes</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
            All transaction payments and deliveries are settled cleanly. When you raise a dispute, it will appear here.
          </p>
        </div>
      ) : (
                ...
                {disputes.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                    <td className="py-3.5 px-4 font-mono font-bold text-slate-900 dark:text-white">
                      <Link href={`/dashboard/buyer/disputes/${d.id}`} className="hover:text-primary transition">
                        {d.disputeNumber}
                      </Link>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <Link
                        ...
                        className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-semibold text-xs transition"
                      >
                        View & Reply →
                      </Link>
                    </td>
